package xg.yxtk.service.sso.iml;
...
import xg.yxtk.mapper.TbUserMapper;
import xg.yxtk.pojo.TbUser;
import xg.yxtk.pojo.TbUserExample;
import xg.yxtk.redis.JedisClient;
import xg.yxtk.service.sso.UserService;
import xg.yxtk.utils.FastJsonConvert;
...
@Service(version = Const.WNDH_SSO_VERSION)
@Transactional
public class UserServiceImpl implements UserService {
    ...
    @Reference(version = Const.WNDH_REDIS_VERSION)
     ...
     */
    @Override
    public XgResult checkUserDate(String data, Integer type, String callback) {

        if (StringUtils.isNotBlank(callback)) {
            return XgResult.ok(callback);
        }
        ...
        switch (type) {
            ...
            default:
                ...
                return XgResult.build(400,"error", "参数错误");
        }

        List<TbUser> list = userMapper.selectByExample(example);

        if (list != null && list.size() > 0) {
            return XgResult.ok(false);
        }

        return XgResult.ok(true);
    }
    ...
    public XgResult register(TbUser user) {
        ...
        if (user == null) {
            return XgResult.build(400, "error", "数据为空");
        }
        ...
        if (usernameb & phoneb & emailb) {
            ...
            try {
                ...
                return XgResult.build(500, "error", "系统异常. 请稍后重试");
            }
            return XgResult.ok();
        }

        return XgResult.build(400, "error", "注册失败. 请校验数据后请再提交数据");
    ...
    @Override
    public XgResult login(TbUser user) {
     ...
     */
    @Override
    public XgResult token(String token, String callback) {
        ...
        return XgResult.build(400, "没有此用户");
    }
     ...
     */
    @Override
    public XgResult logout(String token, String callback) {
        ...
        try {
            ...
            return XgResult.build(400, "没有登录");
        }
     ...
     */
    @Override
    public XgResult validateUser(String isEngaged,String regName,String email,String phone) {
        ...
        return XgResult.build(400,"数据类型错误");
    }
